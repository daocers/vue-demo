  ...
  export default {
    ...
    methods: {
      ...
      findByCondition: async function () {
        let data = await this.doGet("/goods/api/findByCondition?pageNum=" + this.queryForm.pageNum + "&pageSize=" + this.queryForm.pageSize, this.queryForm);
      ...
      commitData: async function () {
        ...
        this.$refs['editForm'].validate(async (valid) => {
          if (!valid) {
            ...
            var res = await this.http('/goods/v1/save', this.dataForEdit, 3000);
            if (res) {
      ...
      async toRemove(idx, row) {
        ...
        this.$confirm('数据删除后无法找回, 是否继续?', '提示', {
          ...
          let data = await this.http("/goods/v1/delete?id=" + row.id);
          if (data == true) {
